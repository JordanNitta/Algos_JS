...
function symmetricDifferences(numbersA, numbersB) {
    ...
    let arryTwo = 0;
    let results = []
    while (arryOne < numbersA.length && arryTwo < numbersB.length) {
        if (numbersA[arryOne] === numbersB[arryTwo]) {
            arryOne++;
            arryTwo++;
        } else if (!results.includes(numbersA[arryOne])) {
            results.push(numbersA[arryOne]);
            arryOne++;
        } else if (!results.includes(numbersB[arryTwo])) {
            results.push(numbersB[arryTwo]);
            arryTwo++;
        } 
    }
    return results;
