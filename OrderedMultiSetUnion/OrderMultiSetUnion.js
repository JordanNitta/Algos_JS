...
function orderedMultisetUnion(sortedA, sortedB) { 
    ...
    let secondArry = 0
    let results = []
    while(firstArry < sortedA.length && secondArry < sortedB.length){
        if(sortedA[firstArry] < sortedB[secondArry]){
            results.push(sortedA[firstArry])
            firstArry++
            console.log(results)
        } else if (sortedA[firstArry] > sortedB[secondArry]){
            results.push(sortedB[secondArry])
            secondArry++
            console.log(results)
        } else {
