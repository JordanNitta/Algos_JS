...
function quickSort(nums) {
  ...
  const leftArry = [];
  const rightArry = []
  // Every element but the last one because we don't want the pivot number part of the for loop
  for (let i = 0; i < nums.length - 1; i++) {
    if(nums[i] < pivot) {
      leftArry.push(nums[i])
    } else {
      rightArry.push(nums[i])
    }
  }
  // Call the function to sort the two sub array and join them together with the pivot
